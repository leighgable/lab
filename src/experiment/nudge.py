...
def compute_topk_sims(q_embs, embeddings_nontrain, k, dist, batch_size):
    ...
    for i in range(int(math.ceil(len(embeddings_nontrain)/batch_size))):
        if dist == "cos":
            curr_topk = torch.topk(torch.matmul(q_embs, torch.t(F.normalize(torch.from_numpy(embeddings_nontrain[i*batch_size:(i+1)*batch_size]).to(q_embs.device)))), k=k, dim=1)
        elif dist == "dot":
...
class NUDGEM:
    ...
    def finetune_embeddings(self, 
        ...
        '''
        embeddings = F.normalize(torch.from_numpy(embeddings).to(self.device))
        ...
        gs = F.normalize(torch.cat(gs, dim=0))
...
class NUDGEN:
    ...
    def _sol_maxm(self, gamma, embeddings, vTc, cTc, gs, zero_updates):
        if gamma != 0:
            ...
            mask = ((embeddings*F.normalize(gs)).sum(axis=-1)>=1-gamma/2).reshape(-1, 1)
